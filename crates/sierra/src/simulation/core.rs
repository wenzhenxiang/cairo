...
use crate::extensions::dict::{self, DictConcreteLibFunc};
...
>(
    ...
    match libfunc {
        ...
        CoreConcreteLibFunc::Dict(DictConcreteLibFunc::New(_)) => {
            if inputs.is_empty() {
        ...
        CoreConcreteLibFunc::Dict(DictConcreteLibFunc::Read(_)) => match &inputs[..] {
            [CoreValue::Dict(map), CoreValue::Felt(key)] => {
                // Returns 0 as a defualt value.
                // TODO(Gil): correct the behavious when dict behaviour is decided when key not
                // found.
        ...
        CoreConcreteLibFunc::Dict(DictConcreteLibFunc::Write(_)) => match &inputs[..] {
